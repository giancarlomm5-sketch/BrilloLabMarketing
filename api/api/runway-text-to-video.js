...
export default async function handler(req,res){
  ...
  try{
    ...
    const r=await fetch('https://api.runwayml.com/v1/text-to-video',{method:'POST',headers:{Authorization:`Bearer ${key}`,'Content-Type':'application/json'},body:JSON.stringify(body)});
    const text=await r.text();res.status(r.status).setHeader('Content-Type',r.headers.get('content-type')||'application/json').send(text);
  }catch(e){res.status(500).json({error:e?.message||'Runway proxy failed'});}
}
